    ...
    function confirmLogin(){
        $.get("/isScan/" + uuid , function(data, status){
            ...
            if(data.indexOf("200") > -1) {
                url = data.substring(data.indexOf('http'), data.lastIndexOf('/'));
                console.log('url ====> ' + url);
...
function webwxinit() {
    $.get('/wxinit', {pass_ticket:base_request.pass_ticket,
                      skey:base_request.Skey,base_request:sessionStorage.getItem('base_request'),
                      url:sessionStorage.getItem('url')}, function(data){
                          init = data;
                          console.log(data);
...
app.controller('myCtrl', function($scope, $http, $location) {
    ...
    $scope.getConstact = function() {
        $.get('/getConstact', {pass_ticket:base_request.pass_ticket, skey:base_request.Skey,
                               url : sessionStorage.getItem('url')}, function(data){
            console.log(data);
            contact = data;
...
function sendMessage(message, toUserName){
    ...
    $.get('/send_msg',{message: message, user_name : user_name,
                       to_user_name: toUserName,
                       pass_ticket: base_request.pass_ticket,
                       base_request: sessionStorage.getItem('base_request'),
                       url : sessionStorage.getItem('url')}, function(data){
        console.log(data);
    });
}
