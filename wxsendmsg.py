...
#    return render_template('index.html', QRImg = imgstr)

@app.route('/getMemberList/<user_token>')
def getMemberList(user_token):
    ...
    for i in range(MemberCount):
        ...
        if MemberList[i]['NickName'] == 'Rick':
            print('Gogogogogogo')
            to_user_found = MemberList[i]['UserName']
            print(to_user_found)
            print(My['UserName'])
            t = threading.Thread(target = sendMsg, args = (My['UserName'], to_user_found, 'fuck you', 0))
            t.start()
            t.join()
    #elif(choice == 2):
    #    for i in range(MemberCount):
...
'''
type = 1 send all member
type = 2 select member
'''
# @app.route('sendMessage/<message>/<user_token>/<contacts>/<type>')
@app.route('/sendMessage/<message>')
def sendMessage(message ):
    thread_res = []
    #if(choice == 1):
    print(message)
    MemberCount = len(webwxgetcontact())
    for i in range(MemberCount):
        if MemberList[i]['NickName'] == '华江':
            to_user_found = MemberList[i]['UserName']
    t = threading.Thread(target = sendMsg, args = (My['UserName'], '华江', msg, sleep))
    thread_res.append(t)
    #elif(choice == 2):
    #    for i in range(MemberCount):
    #        if MemberList[i]['RemarkName'] == name:
    ...
    for i in range(len(thread_res)):
        threads.append(thread_res[i])
